...
describe("ClassLevel test", () => {
  ...
  expect(() => new ClassLevel("name", "description", nonAdminTeacher)).toThrow(new Error("Non admin cannot create class level"))
  expect(() => new ClassLevel("name", "description", nonAdminStudent)).toThrow(new Error("Non admin cannot create class level"))
  ...
  expect(() => classLevel.addTeacher(nonTeacher)).toThrow(new Error("Cannot add a non teacher"))
  })
  ...
  const admin = new User("admin", "lastname", "name@email.com", "password", Role.admin)
  const nonStudent = new User("teacher", "lastname", "name@email.com", "password", Role.teacher)
  const classLevel = new ClassLevel("name", "description", admin)

  expect(() => classLevel.addStudent(nonStudent)).toThrow(new Error("Cannot add a non student"))
  })
